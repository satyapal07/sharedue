    ...
    <div className="flex flex-col min-h-full bg-[#F5F0EB]">
      ...
      <div className="flex flex-col items-center pt-2 pb-6 px-5">
        ...
        <div className="flex gap-2 mt-5 w-full">
          {!isOwed && (
            <button className="flex-1 py-3 rounded-full bg-[#DF5830] text-white text-[13px] font-bold">
              Settle
            </button>
          )}
          {isOwed && (
            <button className="flex-1 py-3 rounded-full bg-[#1A1510] text-white text-[13px] font-bold">
              Remind
            </button>
          )}
          <button className="flex-1 py-3 rounded-full bg-white text-[#1A1510] text-[13px] font-semibold border border-[#E8E2DB]">
            View history
          </button>
        </div>
      </div>
          ...
          <div key={month}>
            ...
            <ul className="px-5">
              {items.map((expense, idx) => {
                ...
                return (
                  <li key={expense.id} className={`flex gap-3 py-3.5 ${idx !== 0 ? "border-t border-[#E8E2DB]" : ""}`}>
      ...
      <FloatingAddButton label="Add expense" expenseHref={`/add-expense?friendId=${id}`} />
    </div>
  );
}
