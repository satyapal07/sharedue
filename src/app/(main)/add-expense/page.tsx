...
function AddExpenseInner() {
  ...
  const initialWithWho: WithType = initialFriendId
    ? { kind: "friend", id: initialFriendId }
    : initialGroupId
    ? { kind: "group", id: initialGroupId }
    : null;

  const [step, setStep] = useState(initialWithWho ? 1 : 0);

  // Step 0
  const [withWho, setWithWho] = useState<WithType>(initialWithWho);
  ...
  const [expenseCurrency, setExpenseCurrency] = useState(homeCurrency.code);
  ...
  // Returns the Avatar element for a member name — uses real Avatar for known friends
  function memberAvatar(name: string) {
    if (name === "You") return <Avatar avatarId={1} />;
    const knownFriend = friends.find((f) => f.name === name);
    if (knownFriend?.avatarId) return <Avatar avatarId={knownFriend.avatarId} />;
    return (
  ...
  return (
    <div className="flex flex-col min-h-full bg-[#F5F0EB]">
          ...
          <>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div>
                ...
                >
                  <span className="text-[15px] font-semibold text-[#1A1510]">{expenseCurrency}</span>
                  <div className="flex items-center gap-1.5">
                    <span className="text-[13px] text-[#9B8F86]">
                ...
                <div className="flex bg-[#E8E2DB] rounded-xl p-0.5">
                  {members.map((m) => (
                    <button
                      ...
                      style={paidBy === m ? { background: "#1A1510", color: "#fff" } : { color: "#9B8F86" }}
                    >
                      {m === "You" ? "You" : m.split(" ")[0]}
                    </button>
                  ))}
          ...
          <>
            ...
            <div className="bg-[#1A1510] rounded-3xl px-5 py-4 text-white">
              ...
              <p className="text-3xl font-black tracking-tight">
                {CURRENCIES.find(c => c.code === expenseCurrency)?.symbol ?? "$"}{parsedAmount.toFixed(2)}
              </p>
              {expenseCurrency !== homeCurrency.code && (
                <p className="text-[11px] text-[#9B8F86] mt-0.5">{expenseCurrency} · conversion applied at settlement</p>
              )}
      ...
      {currencyPickerOpen && (
        <>
          <div className="fixed inset-0 bg-black/50 z-50" onClick={() => setCurrencyPickerOpen(false)} />
          <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[390px] z-50 bg-white rounded-t-3xl px-5 pt-4 pb-20">
            ...
            <p className="text-[11px] font-semibold text-[#9B8F86] uppercase tracking-widest mb-4">Expense currency</p>
            <ul className="flex flex-col">
              {CURRENCIES.map((c, idx) => (
                <li key={c.code}>
                  <button
                    onClick={() => { setExpenseCurrency(c.code); setCurrencyPickerOpen(false); }}
                    className={`w-full flex items-center gap-3 py-3.5 text-left ${idx !== 0 ? "border-t border-[#E8E2DB]" : ""}`}
                  >
