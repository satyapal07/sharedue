...
export default function FriendsPage() {
  ...
  const filterButton = (
    <button
      ...
      className="relative flex items-center gap-1.5 h-8 rounded-full px-3 transition-colors"
    ...
    >
      ...
      <span className="text-[11px] font-semibold" style={{ color: filterOpen || isFiltered ? "#fff" : "#9B8F86" }}>
        {isFiltered ? (filter !== "all" ? FILTERS.find(f => f.value === filter)?.label : "Filtered") : "Filter"}
      </span>
    </button>
  );
  ...
  return (
    <div className="flex flex-col min-h-full bg-[#F5F0EB]">
      ...
      {/* Backdrop */}
      {filterOpen && (
        <div className="fixed inset-0 z-30 bg-black/20" onClick={() => setFilterOpen(false)} />
      )}

      {/* Filter panel */}
      <div className={`relative z-50 overflow-hidden transition-all duration-300 ease-in-out ${filterOpen ? "max-h-36" : "max-h-0"}`}>
        <div className="mx-5 mt-1 mb-3 bg-white rounded-2xl shadow-lg p-3 flex flex-col gap-2.5">
          {/* Search bar */}
          <div className="flex items-center gap-2.5 bg-[#F5F0EB] rounded-xl px-3 py-2.5">
            ...
            {FILTERS.map((f) => (
              <button
                ...
                style={filter === f.value
                  ? { background: "#fff", color: "#1A1510", boxShadow: "0 1px 4px rgba(0,0,0,0.1)" }
                  : { color: "#9B8F86" }}
              >
                {f.label}
              </button>
