...
export default function CategoryIcon({ category, size }: { category: ExpenseCategory; size?: "default" | "lg" }) {
  const iconClass = size === "lg" ? "w-8 h-8 text-[#1A1510]" : "w-6 h-6 text-[#1A1510]";
  ...
  return (
    <div className={`${size === "lg" ? "w-16 h-16 bg-[#E8E2DB] rounded-3xl" : "w-11 h-11 bg-[#F5F0EB] rounded-2xl"} flex items-center justify-center flex-shrink-0`}>
      {icons[category]}
    </div>
  );
}
