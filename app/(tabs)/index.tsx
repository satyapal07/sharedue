...
import { View, Text, ScrollView, TouchableOpacity, TextInput, Modal, StyleSheet, FlatList } from 'react-native';
...
export default function FriendsScreen() {
  ...
  const filtered = friends.filter(f => {
    const matchFilter = filter === 'all' || (filter === 'to-pay' && f.balance < 0) || (filter === 'to-receive' && f.balance > 0);
    return matchFilter && f.name.toLowerCase().includes(search.toLowerCase());
  });
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.cardRow}>
            <View style={styles.cardHalf}>
              <Text style={[styles.cardSubLabel, { color: C.orange }]}>TO PAY</Text>
              <Text style={styles.cardSubAmt}>{fmt(youOwe)}</Text>
            </View>
            <View style={styles.cardHalf}>
              <Text style={[styles.cardSubLabel, { color: '#34d399' }]}>TO RECEIVE</Text>
              <Text style={styles.cardSubAmt}>{fmt(youAreOwed)}</Text>
            </View>
          </View>
        </View>
        ...
        {filtered.length === 0 && <Text style={styles.empty}>No friends match.</Text>}

        {/* Settled toggle */}
        <TouchableOpacity style={styles.settledBtn} onPress={() => setShowSettled(v => !v)}>
          <Text style={styles.settledBtnText}>{showSettled ? 'Hide settled friends' : 'Show settled friends'}</Text>
        </TouchableOpacity>

        {showSettled && settledFriends.map((f, idx) => (
          <View key={f.id} style={[styles.row, idx !== 0 && styles.rowBorder]}>
            <Avatar name={f.name} avatarId={f.avatarId} size={38} />
            <View style={styles.rowContent}><Text style={styles.rowName}>{f.name}</Text><Text style={styles.rowSub}>Settled up</Text></View>
      ...
      <Modal visible={filterOpen} transparent animationType="slide" onRequestClose={() => setFilterOpen(false)}>
        ...
        <View style={[styles.sheet, { paddingBottom: insets.bottom + 20 }]}>
          ...
          <View style={styles.searchBar}>
            ...
            <TextInput value={search} onChangeText={setSearch} placeholder="Search friends…"
              placeholderTextColor={C.mutedLight} style={styles.searchInput} />
