    ...
    <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 60 }}>
        <View style={styles.profile}>
          <CategoryIcon category={group.category} size={64} />
          <Text style={styles.name}>{group.name}</Text>
          <Text style={styles.memberCount}>{group.memberCount} members</Text>
          <Text style={[styles.balance, { color: isOwed ? C.emerald : C.orange }]}>
            {isOwed ? 'You are owed ' : 'You owe '}{fmt(group.balance)}
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  memberCount: { fontSize: 13, color: C.muted, marginTop: 2 },
  balance: { fontSize: 16, fontWeight: '600', marginTop: 4 },
  membersRow: { paddingHorizontal: 20, gap: 8, marginBottom: 20 },
  memberPill: { backgroundColor: C.divider, borderRadius: 50, paddingHorizontal: 14, paddingVertical: 8 },
  memberText: { fontSize: 13, fontWeight: '600', color: C.dark },
